...
product_data = df[['product_id', 'category', 'price', 'quantity', 'shopping_mall']]
...
customer_product_matrix = train_data.pivot_table(index='customer_id', columns='product_id', values='age')
...
def predict_ratings(customer_id, product_id):
    ...
    if product_id in customer_product_matrix:
        ...
        weighted_sum = np.dot(customer_similarity[customer_idx], customer_product_matrix_filled.iloc[:, product_idx])
...
def predict_content_based(customer_id, product_id):
    customer_products = train_data[train_data['customer_id'] == customer_id]['product_id']
    if not customer_products.empty and product_id in product_data['product_id'].values:
        ...
        customer_product_idxs = [product_data[product_data['product_id'] == pid].index[0] for pid in customer_products]
        similarities = product_similarity[product_idx, customer_product_idxs]
        customer_ratings = customer_product_matrix_filled.loc[customer_id, customer_products].values
...
def recommend_products(customer_id, top_n=5):
    ...
    predictions = [(pid, hybrid_predict(customer_id, pid)) for pid in product_ids]
